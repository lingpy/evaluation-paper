"""
Stage 4:
Correlation, Mantel test, neighbor-join tree, 
Generalized Robinson-Foulds Distance, Generalized Quartet Distance.
"""
import csv
import numpy as np
import itertools
...
# Correlation
Concepts = {}
with open("results/cognate-set-comparison.tsv", "r") as file:
...
for i, j in zip(files, files_variable):
    with open("/".join(["results", i]), "r") as f:
        tmp = f.readlines()
        doculect = []
        tmp_matrix = []
