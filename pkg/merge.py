...
concepts = {c.concepticon_gloss: c.concepticon_id for c in
        concepticon.conceptlists['Chen-2012-888'].concepts.values() if
        c.concepticon_id}

languages = {
    row[0]: [row[3], row[4]]
    for row in csv2list(data_path.joinpath('languages.tsv').as_posix(), strip_lines=False)[1:]
}
...
ratliff = Wordlist.from_cldf(
    ...
    filter=lambda row: row["language_id"] in languages.keys() and row['concept_concepticon_id']
    )
...
for language in wl.cols:
    ...
    for concept, idxs in rows.items():
        ...
        if ratliff_tmp[concept]:
            ...
            for idx in idxs:
                for i, morpheme in enumerate(
                        basictypes.lists(wl[idx, 'tokens']).n):
